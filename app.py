...
class MedicalReportAgent:
    def __init__(self, api_key=None, model="gpt-3.5-turbo", temperature=0.7, base_url=None):
        ...
        self.prompt_template = ChatPromptTemplate.from_template(
            ...
            2. 请将回答分成多个要点，每个要点使用数字序号开头。
            3. 每个要点必须独占一行，不要将多个要点挤在同一行。
            4. 每个要点之间必须使用至少一个空行分隔，使排版更加清晰。
            5. 对于重要信息，可以适当使用换行来突出显示。
            6. 回答要简洁明了，避免冗长的段落。
            7. 请严格按照以下格式输出（这是唯一正确的格式）：
               1. 要点一的内容，包含相关信息和细节
               ...
               4. 以此类推
            8. 确保每个要点都有明确的主题，内容简洁明了。
            9. 必须严格按照上述格式输出，否则将被视为回答错误。
